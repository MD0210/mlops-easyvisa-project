...
from easy_visa.constants import APP_HOST, APP_PORT
from easy_visa.pipeline.prediction_pipeline import EasyvisaData, EasyvisaClassifier
...
class DataForm:
    ...
    async def get_easyvisa_data(self):
...
async def index(request: Request):
    ...
    return templates.TemplateResponse(
            "easyvisa.html",{"request": request, "context": "Rendering"})
...
async def predictRouteClient(request: Request):
    try:
        ...
        easyvisa_df = easyvisa_data.get_easyvisa_input_data_frame()

        model_predictor = EasyvisaClassifier()

        value = model_predictor.predict(dataframe=easyvisa_df)[0]
